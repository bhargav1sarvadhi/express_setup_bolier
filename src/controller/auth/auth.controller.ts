...
class AuthController {
    ...
    async login(req, res, next) {
     ...
     async GenrateOTP(){
        ...
        const expiresIn = Math.floor((currentTime.getTime() + 10 * 60 * 1000) / 1000);
    ...
    async forgetPassword(req, res, next) {
        try {
            ...
            if (!user) {
                throw new AppError(RES_TYPES.USER_NOT_FOUND, ERRORTYPES.NOT_FOUND)
            }
            const { otp, expiresIn } = await authController.GenrateOTP()
            const checkotp = await db.otpModel.findOne({ where: { userId: user.id } })
            if (checkotp) {
               saveOtp = await db.otpModel.update({otp, expiresIn},{where:{userId: user.id}})
            } else {
              saveOtp = await db.otpModel.create({otp, expiresIn,userId:user.id}) 
            }
            return res.status(200).json({
                    success: true,
    ...
    async checkValidateOtp(req, res, next) {
        try {
             ...
             const { body: { otp } } = req;
            const findOtp = await db.otpModel.findOne({ where: { otp } });
            console.log(currentTime);
            console.log(findOtp.expiresIn);
            if (!findOtp || currentTime > findOtp.expiresIn) {
                throw new AppError(RES_TYPES.NOT_VALIDATE_OTP, ERRORTYPES.INVALID_REQUEST)
            }
    ...
    async resetPassword(req, res, next) {
       try {
           ...
           const decodeToken:jwt.JwtPayload= await TokenController.decodeToken(token, next) as jwt.JwtPayload;
           const [updatePassword] = await db.userModel.update({ password }, { where: { id:decodeToken.id } });
           if (!updatePassword || updatePassword == 0) {
               throw new AppError(RES_TYPES.ID_NOT_FOUND,ERRORTYPES.NOT_FOUND)
           }
           const deleteOtp = await db.otpModel.destroy({ where: { userId: decodeToken.id } });
            return res.status(200).json({
                success: true,
